...
=head1 Usage
  ...
  --resource <str>  set the required resource used in qsub -l option, default nodes=1:ppn=1,mem=2G,walltime=10:00:00
...
=head1 Exmple
  ...
  perl qsub-sge.pl --resource nodes=1:ppn=1,mem=2G,walltime=10:00:00 ./work.sh

  7.recommend combination of usages for common applications (I think this will suit for 99% of all your work)
  perl qsub-sge.pl --queue all.q --resource nodes=1:ppn=1,mem=2G,walltime=10:00:00 -maxjob 10 --reqsub ./work.sh
